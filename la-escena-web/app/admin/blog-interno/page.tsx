...
import { getInternalPosts } from "@/lib/sanity"

export default async function AdminBlogInternoPage() {
  await requireAdmin()
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-heading text-4xl">Noticias Internas</h1>
          <p className="text-sm text-zinc-500 mt-1">Visibles solo para artistas en /artista/noticias</p>
        </div>
        <Link href="/admin/blog-interno/create">
          ...
          <tbody>
            {posts.map((post: any) => (
              <tr key={post._id} className="border-b border-zinc-50 last:border-0 hover:bg-zinc-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <Link href={`/admin/blog-interno/${post._id}`}>
